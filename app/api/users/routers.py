...
async def login_for_access_token(user_create: UserCreate, db: Session = Depends(PostgresqlManager.get_db)):
    ...
    Allows the external API user to register a new user account.

    - The provided user creation data must meet the specified requirements.
    - Upon successful registration, the user information is returned.

    Returns:
    - dict: User information after successful registration.
    
    Raises:
    - HTTPException: If passwords do not match or registration fails.
    """
    try:
